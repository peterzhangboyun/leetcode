                ...
                for j in range(i + 1, len(temp)):
                    ...
                    while k < (len(temp[i]) - 1):
                        ...
                        while l < (len(temp[j]) - 1):
                            inters = findIntersection(temp[i][k][0], temp[i][k][1], temp[i][k + 1][0],
                                                      temp[i][k + 1][1], temp[j][l][0], temp[j][l][1],
                                                      temp[j][l + 1][0], temp[j][l + 1][1])
                            if inters != None:
                                ...
                                while lines[i][q + 1] != '\n':
                                    if min([lines[i][q][0], lines[i][q + 1][0]]) <= inters[0] <= max(
                                            [lines[i][q][0], lines[i][q + 1][0]]) and min(
                                            [lines[i][q + 1][1], lines[i][q][1]]) <= inters[1] <= max(
                                            [lines[i][q][1], lines[i][q + 1][1]]):
                                        if lines[i][q] != inters and lines[i][q + 1] != inters:
                                            lines[i].insert(q + 1, inters)
                                        break
                                    q = q + 1
                                while lines[j][z + 1] != '\n':
                                    if min([lines[j][z + 1][0], lines[j][z][0]]) <= inters[0] <= max(
                                            [lines[j][z][0], lines[j][z + 1][0]]) and min(
                                            [lines[j][z + 1][1], lines[j][z][1]]) <= inters[1] <= max(
                                            [lines[j][z][1], lines[j][z + 1][1]]):
